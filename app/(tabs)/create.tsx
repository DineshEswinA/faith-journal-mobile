...
export default function CreateScreen() {
  ...
  const uploadCoverImage = async () => {
    if (!user) {
      Alert.alert('Sign In Required', 'You must be signed in to upload a cover image.');
      return;
    }
    ...
    try {
      ...
      const fileExt = asset.uri.split('.').pop()?.toLowerCase() || 'jpg';
      const contentType = fileExt === 'jpg' ? 'image/jpeg' : `image/${fileExt}`;
      const filePath = `${user.id}/covers/${Date.now()}.${fileExt}`;
      ...
      Alert.alert('Upload Failed', error?.message || 'Could not upload the cover image. Please try again.');
    } finally {
      setUploadingCoverImage(false);
    }
  };

  const buildPostPayload = (status: 'draft' | 'published') => ({
  ...
  const handleSaveDraft = async () => {
    const hasDraftContent = !!title.trim() || !!content.trim() || !!coverImage || assets.length > 0;
    if (!hasDraftContent) {
      Alert.alert('Empty Draft', 'Add a title, content, cover image, or attachment before saving a draft.');
      return;
    }
    if (!user) {
      Alert.alert('Sign In Required', 'You must be signed in to save a draft.');
      return;
    }
    ...
    const { data, error } = currentDraftId
      ? await supabase
          .from('posts')
          .update(buildPostPayload('draft'))
          .eq('id', currentDraftId)
          .eq('author_id', user.id)
          .select('id')
          .single()
      : await supabase.from('posts').insert(buildPostPayload('draft')).select('id').single();
    setSavingDraft(false);

    if (error) {
      Alert.alert('Draft Error', error.message);
      return;
    }

    setCurrentDraftId(data?.id || currentDraftId);
  ...
  const handlePost = async () => {
    ...
    const { error } = currentDraftId
      ? await supabase
          .from('posts')
          .update(buildPostPayload('published'))
          .eq('id', currentDraftId)
          .eq('author_id', user.id)
      : await supabase.from('posts').insert(buildPostPayload('published'));
  ...
  const canPublish = !!title.trim() && !!stripEditorSyntax(content) && !loading && !savingDraft;
  const canSaveDraft = (!!title.trim() || !!content.trim() || !!coverImage || assets.length > 0) && !loading && !savingDraft;
          ...
          <View className="px-6 py-6 flex-row justify-between items-center">
            <View className="flex-1 pr-4">
              ...
              <View className="flex-row items-center">
                ...
                <Text className="text-[9px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider ml-1.5">{draftStatusText}</Text>
              </View>
            </View>
            <View className="flex-row items-center">
              <TouchableOpacity className="mr-5 min-w-20 items-center" onPress={handleSaveDraft} disabled={!canSaveDraft}>
                {savingDraft ? (
                  <ActivityIndicator color="#047857" size="small" />
                ) : (
                  <Text className={`text-xs font-bold ${canSaveDraft ? 'text-slate-600 dark:text-slate-400' : 'text-slate-300 dark:text-slate-600'}`}>
                    Save Draft
                  </Text>
                )}
              </TouchableOpacity>
            ...
            <View className="mb-6">
              <TouchableOpacity
                className={`w-10 h-10 rounded-full items-center justify-center border ${
                  coverImage
                    ? 'bg-[#E8F3EE] dark:bg-[#0D2318] border-[#047857]/20'
                    : 'bg-white dark:bg-[#1A1A1A] border-slate-200 dark:border-slate-700'
                }`}
                onPress={() => setShowCoverUploader((value) => !value)}
                hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}
              >
                <ImageIcon size={18} color={coverImage ? '#047857' : colors.icon} />
              </TouchableOpacity>

              {showCoverUploader ? (
                <View className="mt-4 bg-white dark:bg-[#1A1A1A] rounded-2xl border border-slate-100 dark:border-slate-800 overflow-hidden">
                  {coverImage ? (
                    <View>
                      <View className="w-full h-44 bg-slate-200 dark:bg-slate-700">
                        <Image source={{ uri: coverImage }} className="w-full h-full" resizeMode="cover" />
                      </View>
                      <View className="flex-row items-center justify-between p-4">
                        <Text className="text-[11px] font-bold text-[#047857] uppercase tracking-[1.5px]">Cover Image</Text>
                        <View className="flex-row items-center gap-x-4">
                          <TouchableOpacity onPress={uploadCoverImage} disabled={uploadingCoverImage}>
                            {uploadingCoverImage ? (
                              <ActivityIndicator size="small" color="#047857" />
                            ) : (
                              <Text className="text-xs font-bold text-slate-700 dark:text-slate-300">Change</Text>
                            )}
                          </TouchableOpacity>
                          <TouchableOpacity onPress={() => setCoverImage(null)} className="w-8 h-8 rounded-full bg-slate-100 dark:bg-[#222222] items-center justify-center">
                            <X size={15} color={colors.icon} />
                          </TouchableOpacity>
                        </View>
                      </View>
                    </View>
                  ) : (
                    <TouchableOpacity className="h-36 items-center justify-center" onPress={uploadCoverImage} disabled={uploadingCoverImage} activeOpacity={0.8}>
                      {uploadingCoverImage ? (
                        <ActivityIndicator size="small" color="#047857" />
                      ) : (
                        <View className="items-center">
                          <View className="w-11 h-11 rounded-full bg-slate-100 dark:bg-[#222222] items-center justify-center mb-3">
                            <ImageIcon size={20} color={colors.icon} />
                          </View>
                          <Text className="text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-[1.5px]">Upload Cover Image</Text>
                        </View>
                      )}
                    </TouchableOpacity>
                  )}
